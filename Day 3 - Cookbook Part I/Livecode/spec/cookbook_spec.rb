...
describe 'Cookbook', if: cookbook_helper.file_and_class_valid? do
  ...
  describe 'without CSV' do
    ...
    before do
      @cookbook = if Cookbook.instance_method(:initialize).arity == 1
                    Cookbook.new(csv_path)
                  else
                    Cookbook.new
                  end
    end
  ...
  describe 'with CSV' do
    ...
    describe '#all' do
      ...
      it 'should return array of Recipe instances' do
        ...
        expect(first_recipe).to be_instance_of(Recipe), lambda {
                                                          "Cookbook should store Recipe instances, not #{first_recipe.class}"
                                                        }
      end
    end
